...
import { ERROR_CODES, type ErrorCode } from "../src/contracts";
import {
  createInitialCertificationProgress,
  updateCertificationProgress,
  type ProficiencyScore
} from "../src/sim";

function emptyRejectedMap(): Record<ErrorCode, number> {
  return Object.fromEntries(ERROR_CODES.map((code) => [code, 0])) as Record<ErrorCode, number>;
}

function buildScore(overrides: Partial<ProficiencyScore>): ProficiencyScore {
  return {
    pickAccuracy: 1,
    totalRejected: 0,
