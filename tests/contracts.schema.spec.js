...
describe("events schema validation", () => {
    ...
    it("fails invalid events", () => {
        ...
        expect(validateEvent(invalidMissingBarcodeInPayload).ok).toBe(false);
        expect(validateEvent(invalidUnknownType).ok).toBe(false);
    });
    it("fails event with unknown top-level property", () => {
        ...
        expect(validateEvent(invalidExtraTopLevel).ok).toBe(false);
    });
});
