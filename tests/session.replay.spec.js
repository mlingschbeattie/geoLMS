...
import { applyAction, createSession, getActivePickTask, loadScenarioFromFile } from "../src/sim";
function createActionFactory(traineeId, sessionId) {
    let index = 0;
    return {
        build(type, payload) {
            index += 1;
...
function runActions(initial, actions) {
